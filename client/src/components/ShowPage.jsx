...
export default function ShowPage() {
  ...
  if (!show) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p style={{ color: '#94a3b8' }}>Loading...</p></div>
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: '32px', paddingTop: '20px' }}>
        <h1 style={{ fontSize: '32px', fontWeight: 900, color: '#7c3aed' }}>SetWaves</h1>
        <h2 style={{ fontSize: '20px', marginTop: '8px' }}>{show.displayName}</h2>
        <div style={{ display: 'inline-block', background: '#2d2d3d', borderRadius: '20px', padding: '6px 16px', marginTop: '12px', fontSize: '14px' }}>
          {tokens} token{tokens !== 1 ? 's' : ''}
        </div>
      </div>

      {success && <div style={{ background: '#14532d', border: '1px solid #4ade80', borderRadius: '8px', padding: '12px 16px', marginBottom: '16px', color: '#4ade80', textAlign: 'center' }}>Request sent!</div>}
          ...
          <form onSubmit={requestSong} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {show.songs.length > 0 && (
              <select value={selectedSong} onChange={e => { setSelectedSong(e.target.value); setCustomSong('') }}>
                ...
                {show.songs.map(s => <option key={s.id} value={s.title}>{s.title}{s.artist ? ' - ' + s.artist : ''}</option>)}
              </select>
            )}
            <input placeholder="Or type any song..." value={customSong} onChange={e => { setCustomSong(e.target.value); setSelectedSong('') }} />
            <button type="submit" className="btn-primary" disabled={submitting}>{submitting ? 'Sending...' : 'Request (1 token)'}</button>
          </form>
        </div>
      ) : (
        <div className="card" style={{ marginBottom: '24px', textAlign: 'center' }}>
          <p style={{ color: '#94a3b8', marginBottom: '4px' }}>Get tokens to request songs</p>
        </div>
      )}
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {packages.map(pkg => (
            <button key={pkg.id} onClick={() => buyTokens(pkg)}
              style={{ background: '#1a1a24', border: '1px solid #7c3aed', borderRadius: '10px', padding: '14px 16px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#e2e8f0' }}>
              <span style={{ fontWeight: 600 }}>{pkg.name} - {pkg.tokens} tokens</span>
              <span style={{ color: '#7c3aed', fontWeight: 700 }}>${(pkg.price / 100).toFixed(2)}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {show.queue.length > 0 && (
        <div style={{ marginTop: '32px' }}>
          <h3 style={{ marginBottom: '12px', fontWeight: 600, color: '#94a3b8' }}>Up Next</h3>
          {show.queue.map(item => (
            <div key={item.id} style={{ padding: '10px 14px', borderLeft: '3px solid #7c3aed', marginBottom: '8px', background: '#1a1a24', borderRadius: '0 8px 8px 0' }}>
              <p style={{ fontWeight: 600 }}>{item.songTitle}</p>
              <p style={{ fontSize: '12px', color: '#94a3b8' }}>requested by {item.requester}</p>
            </div>
          ))}
        </div>
      )}
